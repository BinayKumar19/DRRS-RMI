...
public class DorvalServerImpl extends UnicastRemoteObject
    ...
    public String bookRoom(String studentId, String campusName, Integer roomNumber, String date, String timeSlot)
        ...
        try {
            if (campusName.equals("Dorval")) {
                sBookingStatus = bookRoomDorval(studentId, roomNumber, date, timeSlot);
    ...
    public String cancelBooking(String studentId, String bookingID)
        ...
        if (bookingID.charAt(0) == 'D') {
            sBookingStatus = cancelBookingDorval(studentId + bookingID);
        } else if (bookingID.charAt(0) == 'W' || bookingID.charAt(0) == 'K') {
            try {
                DatagramSocket clientSocket = new DatagramSocket();
                InetAddress IPAddress = InetAddress.getByName("localhost");
                byte[] sendData = new byte[1024];
